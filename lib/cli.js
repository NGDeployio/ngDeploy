...
function findToken () {
    ...
    try {
        if (fs.existsSync(pkgConfig) && (sys = JSON.parse(fs.readFileSync(pkgConfig))) && sys.accountToken) {
    ...
    try {
        if (fs.existsSync(globalConfig) && (sys = JSON.parse(fs.readFileSync(globalConfig))) &&  sys.accountToken) {
            return sys.accountToken;
        }
    }catch (err) {

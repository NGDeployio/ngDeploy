...
function createConfiguration(app) {
  ...
  system.accessToken = app.accessToken;
...
>>>>>>> master
  authedRequest(accountToken)
      .get({
          ...
          if (err) {
            logger.info(err);
            return utils.fatal('Error retrieving apps. ' + err, 1);
          }
          if(body.error){
              logger.info(body.error);
              return utils.fatal('Error retrieving apps. ' + body.error, 1);
<<<<<<< HEAD
          }
          if( body && body.length > 0 && body.forEach){
...
function _setEnv(appId, stage, kv) {
  ...
  if (!appId) { return utils.fatal('Need to set the id. -appId ', 1); }

  if (!stage) { return utils.fatal('Need to set the stage. -stage [development|staging|production] '); }

  if (!kv)   { return utils.fatal('Need to set the key value. -kv key=value'); }
...
=======
            return setAccountToken(result.accountToken, true);
>>>>>>> master
   });
}

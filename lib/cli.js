...
function header() {
    utils.logSuccess(' ngdeploy: ' + pkg.description + ' (v' + pkg.version + ')');
}
...
function clean(opts) {
  ...
  files.forEach(function (file) {
      if (fs.existsSync(file)) {
        fs.unlinkSync(file);
        logger.info('Remove: ', chalk.yellow(file), chalk.green(' success'));
      }else {
        logger.info('Remove: ', chalk.yellow(file), chalk.red(' fail'));
        removedAll = false;
      }
    });
...
function push() {
  ...
  authedRequest.get({
    ...
    uploader.on('end', function () {
        ...
        logger.info('find your app at: ', getUrl('development'));
      });
  });
}
...
function addDomain(data, accountToken) {
    var obj;

    if( ! (data.domain && data.id )){
        return fatal("Missing domain or id. ", 1);
    }

    obj = {domain: data.domain, id: data.id};
    ...
    if(accountToken === undefined){
        return fatal("Could not resolve account token. ", 1);
    }
    ...
    authedRequest.post({
        ...
        if (error) {
            return fatal('Failed to create app: ' + data, 1);
        }

        if (body.error) {
            return fatal('Error creating app: '+ body.error, 1);
        } else if (body.response) {
            return logger.info(body.response);
        }
    });
...
function additionalHelp() {
  ...
  logger.info(chalk.yellow('1.    $ ngdeploy set -g -a <AccountToken>'));
  ...
  logger.info('');
  logger.info('Example: ');
  logger.info(chalk.cyan('1.    $ ngdeploy set -g -a 123456789'));
...
function _list(accountToken) {
  ...
  request
      .get({
          ...
          if (err) {
            logger.info(err);
            return fatal('Error retrieving apps. ' + err, 1);
          }
          if(body.error){
              logger.info(body.error);
              return fatal('Error retrieving apps. ' + body.error, 1);
          }
          var items = body || [];
          logger.info('info','Listing apps ........');
          logger.info(chalk.bold('id') +  '..' + 'Name' + '..' + 'Role ');
          items.forEach(function (item) {
              logger.info(chalk.bold(item.apps.id) + '...' + item.apps.name + '...' + item.teams.type);
            });
        });
}
  ...
  authedRequest
      .post({
          ...
          if (body.response !== undefined) {
            ...
            logger.info(chalk.green('Check it out at ' + getUrl(target)));
          }
        });
}

...
function readngdeploy(){
    var _global = getUserHome()+"/.ngdeploy";

    if(fs.existsSync(_global)){
        try{
            var sys = JSON.parse(_global);
            system.accesstoken = sys.accessToken;
        }catch(err){
...
function setAccessKey(accessToken,global){
    var homePath = ".ngdeploy";
    if(global){
        homePath = getUserHome()+"/.ngdeploy";
    }
    try{
        var sys = JSON.parse(fs.readFileSync(homePath));
        sys["accessToken"] = accessToken;
        fs.writeFileSync(homePath, JSON.stringify(sys));
        return true;
    }catch(e){
        fatal("Failed to write into user home directory at "+ homePath,1);
        return false;
    }
}

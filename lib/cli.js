...
function login (opts){
    ...
    console.log(chalk.bold("Open this link in a browser to register: ")+
                chalk.cyan(chalk.bold("https://ngdeploy.com/express")));

    prompt.start({colors:true});

    open("http://ngdeploy.com/#!/user/accounts");

    prompt.get(loginSchema, function (err, result) {
        if (err) {
            return fatal(err, 1);
        }
        setAccountToken(result.accountToken, !!opts.global); // Returns true if global exists.
    });
}
